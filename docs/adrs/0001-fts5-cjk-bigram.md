...
## P6 收尾课补记（2026-08-05）：中文查询串长分流

hybrid 课发现的记账边界正式修复：原查询构造把每个连续中文串整体做成 bigram
短语原子，句子级查询等价于整句子串精确匹配（全有或全无）。修复为按串长分流
（配置 `retrieve.fts5_phrase_max_run`，默认 6）：

- **≤6 字**（术语形态，如 `区块链`/`图神经网络层`）：保持 bigram 短语，
  连续命中等价于子串匹配，保精度；
- **>6 字**（句子形态）：拆 bigram OR 词袋，靠 BM25 排序保召回——句子里
  的低频术语 bigram 因 IDF 自然主导排序；
- 单字前缀 `"字"*` 兜底不变。

pipeline 课的多查询池化直接受益（query rewrite 产出的中文子查询多为句子形态）。
